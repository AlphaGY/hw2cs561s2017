...
#      print str(kb[i][j].prefix) + " " + str(kb[i][j].guest) + " " + str(kb[i][j].table) + " ",
# print "\n"
...
# check if two literals are the same symbol
...
def pl_resolve(complementary, clause1, clause2):
    ...
    for comp in complementary:
        resolvent = []
        for literal1 in clause1:
            if not is_same_symbol(comp, literal1):
